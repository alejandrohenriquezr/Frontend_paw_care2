...
def agendar():
    ...
    for hora in range(9, 19):
        ...
        for _, vet in df_staff.iterrows():
            correo_vet = vet["correo"]
            reservado = df_reservas[
                (df_reservas["hora"] == hora_str) & (df_reservas["medico_que_atendio"] == correo_vet)
            ]
            if reservado.empty:
                espec_ids = [e for e in vet["especialidades"].split(",") if e != "0"]
                especialidades = df_especialidades[df_especialidades["id_especialidad"].astype(str).isin(espec_ids)]["especialidad"].tolist()

                info_vet = df_staff_all[df_staff_all["correo"] == correo_vet].iloc[0]
                disponibilidad.append({
                    "hora": hora_str,
                    "correo": correo_vet,
                    "especialidades": especialidades,
                    "id_veterinario": vet["id_veterinario"],
                    "nombres": info_vet["nombres"],
                    "apellidos": info_vet["apellidos"],
                    "sexo": info_vet["sexo"],
                    "area_interes": info_vet["area_interes"],
                })
...
@app.route("/insertar_reserva", methods=["POST"])
@login_required
def insertar_reserva():
    data = request.get_json()
    df = pd.read_csv("data/reservas.csv", sep=";")
    id_reserva = df["id_reserva"].max() + 1 if not df.empty else 1

    nueva_reserva = {
        "id_reserva": id_reserva,
        "id_clinica": data["id_clinica"],
        "correo_cliente": session["user"]["email"],
        "mascota": data["id_mascota"],
        "fecha": data["fecha"],
        "hora": data["hora"],
        "estado": 1,
        "fecha_agrega_reserva": datetime.now().strftime("%d/%m/%Y"),
        "hora_agrega_reserva": datetime.now().strftime("%H:%M"),
        "correo_veterinario": data["correo_veterinario"]
    }

    df = pd.concat([df, pd.DataFrame([nueva_reserva])], ignore_index=True)
    df.to_csv("data/reservas.csv", sep=";", index=False)

    msg = Message("Confirmación de tu reserva", sender="alhen1970@gmail.com", recipients=[session["user"]["email"]])
    msg.body = f"""Hola,

Tu reserva ha sido confirmada para el día {data["fecha"]} a las {data["hora"]}.
Veterinario: {data["correo_veterinario"]}
